...
// export const VARIABLE_EXP_REG = () => /^[a-zA-Z\$_][\w\$]*$/;
...
export const EXPRESSION_REG = () => /^([\'\"]?[a-zA-Z-\.]+[\'\"]?)\s+([=|>|<]{2,3})\s([\'\"]?[a-zA-Z-0-9]+[\'\"]?)$/
...
export const getFirstExp = (exp: string) => {
    const regExpMatchArray = resolveExpression(exp)
    if (regExpMatchArray) {
        return regExpMatchArray[1].toString()
    }
    return exp
}
